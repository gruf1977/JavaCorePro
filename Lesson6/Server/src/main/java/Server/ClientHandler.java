    ...
    public ClientHandler(final Server server, Socket socket){
        ...
        try {
            ...
            if (Server.COUNT_CLIENTS <= clients.size()) {
                LOGGER.info("Srever.sendMsg: /serverMaxConnect");
                                ...
                                try {
                                    ...
                                    while (true) {
                                        ...
                                        if (str.startsWith("/auth")) {
                                            LOGGER.info("Client.sendMsg: /auth");
                                            ...
                                            if (newNick != null) {
                                                nick = newNick;
                                                sendMsg("/authok " + nick);
                                                LOGGER.info("Server.sendMsg: /authok " + nick);
                                                server.subscribe(ClientHandler.this);
                                                LOGGER.info("Server.sendMsg: /sysinfo Welcome " + nick);
                                                sendMsg("/sysinfo Welcome " + nick);
                                                LOGGER.info("Server.sendMsg: /sysinfo /info - справочник команд");
                                                ...
                                                LOGGER.info("Server.sendMsg: /sysinfo Err: Неверный логин/пароль");
...
//            .start();
    ...
    public void sendMsg(String msg) {
        LOGGER.info("Server.sendMsg: " + msg);
        try {
            out.writeUTF(msg);
        } catch (IOException e) {
            e.printStackTrace();
